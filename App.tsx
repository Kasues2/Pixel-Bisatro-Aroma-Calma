...
import { Coins, Trophy, UtensilsCrossed, Volume2, VolumeX, Users, ArrowRight, Copy, Clock, Calendar, TrendingUp, PlayCircle, Save, Star } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [gameRunId, setGameRunId] = useState('initial'); 
  const [hasSaveFile, setHasSaveFile] = useState(false);
  const [aiLoading, setAiLoading] = useState(false); // UI State for AI
  ...
  const continueGame = () => {
      ...
      if (savedData) {
          ...
          setGameState(prev => ({
              ...
              timeRemaining: GAME_CONFIG.DAY_DURATION_SECONDS,
              gameMode: 'SOLO',
              dailyReview: undefined // Clear old review on load
          }));
  ...
  // --- MULTIPLAYER SETUP ---
  // (Kept same as before, omitted for brevity if no changes, but including full file per instruction)
  ...
  const handleStartService = () => {
    ...
    setGameState(prev => ({
      ...
      phase: 'SERVICE',
      timeRemaining: GAME_CONFIG.DAY_DURATION_SECONDS,
      dailyReview: undefined // Reset review
    }));
    setOrders([]);
  ...
  const checkEndOfDay = async (currentState: GameState) => {
      ...
      if (success) {
          ...
          // Trigger AI Review
          if (currentState.gameMode === 'SOLO') {
             setAiLoading(true);
             const review = await aiService.generateDailyReview(
                 currentState.day, 
                 currentState.money, 
                 currentState.dailyTarget, 
                 currentState.hygiene,
                 currentState.score
             );
             setAiLoading(false);
             setGameState(prev => ({ ...prev, dailyReview: review }));
          }
  ...
  // --- RENDERERS (Menu & Lobby Omitted from snippet but kept in file) ---
       ...
       {gameState.phase === 'EOD_REPORT' && (
           <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
               <div className="bg-stone-100 p-8 rounded-sm border-t-8 border-stone-800 text-center shadow-2xl max-w-md w-full transform rotate-1 relative paper-shadow">
                   ...
                   <div className="space-y-4 mb-4 font-pixel text-xl">
                   ...
                   {/* AI REVIEW SECTION */}
                   <div className="bg-yellow-50 p-3 rounded border border-yellow-200 mb-6 relative">
                      <h3 className="font-pixel text-stone-800 font-bold text-sm mb-1 flex items-center gap-1">
                          <Star size={12} className="text-yellow-500 fill-current" /> Crítico Gastronômico
                      </h3>
                      {aiLoading ? (
                          <div className="text-stone-400 font-pixel text-sm italic animate-pulse">Escrevendo avaliação...</div>
                      ) : (
                          <p className="font-pixel text-stone-600 text-sm leading-tight italic">
                              "{gameState.dailyReview || 'Sem avaliação disponível.'}"
                          </p>
                      )}
                   </div>
      ...
      <div className="max-w-7xl mx-auto p-4 flex flex-col h-screen relative z-10">
        ...
        <div className="flex flex-1 gap-6 overflow-hidden">
                  ...
                  return (
                    <div key={order.id} className={`w-full h-full bg-[#fff9c4] text-stone-900 shadow-lg flex flex-col relative overflow-hidden transform ${rotation} origin-top transition-all duration-300 hover:scale-105 paper-shadow rounded-b-sm`}>
                      ...
                      <div className="h-1.5 bg-stone-300 w-full z-10 border-y border-stone-400 shrink-0">
                         <div className={`h-full transition-all duration-1000 linear ${order.patience < 30 ? 'bg-red-500' : 'bg-green-600'}`} style={{ width: `${order.patience}%` }}/>
                      </div>
                        ...
                        <div className="flex-1 flex flex-col justify-center min-h-0">
                          <div className="flex flex-wrap gap-2 justify-center">
                             {recipe?.ingredients.map((ing, i) => (
                               <div key={i} className={`w-7 h-7 rounded border border-black/20 shadow-sm flex items-center justify-center text-xs font-bold font-pixel text-white ${INGREDIENTS[ing].color}`} title={INGREDIENTS[ing].name}>{ing}</div>
                             ))}
                          </div>
                        </div>
                   ...
                   {activeStationId === station.id && station.state === 'empty' && (
                     <div className="absolute inset-0 bg-stone-900/90 z-20 rounded shadow-2xl flex flex-col items-center p-3 animate-in fade-in zoom-in duration-200 border-2 border-stone-600 backdrop-blur-sm">
                        <div className="flex justify-between w-full items-center mb-2 px-1 shrink-0">
                           ...
                           <button onClick={(e) => { e.stopPropagation(); setActiveStationId(null); }} className="text-xs text-red-300 hover:text-white uppercase tracking-widest transition-colors font-bold">[X]</button>
                        </div>
                        <div className="grid grid-cols-2 gap-2 w-full flex-1 min-h-0">
                          {RECIPES.map(r => (
                            <button key={r.id} onClick={(e) => { e.stopPropagation(); handleRecipeSelect(r.id); }} className="relative overflow-hidden group bg-stone-800 rounded border border-stone-600 hover:border-amber-400 transition-all flex flex-col justify-end items-center text-left">
                              ...
                              <div className="z-10 w-full p-2 flex flex-col items-center">
                                ...
                                <div className="flex gap-0.5 justify-center flex-wrap max-w-full">
                                    {r.ingredients.map((ing, i) => (<div key={i} className={`w-1.5 h-1.5 rounded-full ${INGREDIENTS[ing].color} border border-white/20`} />))}
                                </div>
                 ...
                 <div className="flex flex-wrap gap-2 justify-center">
                    {Object.values(INGREDIENTS).map(ing => (
                      <div key={ing.key} className="flex flex-col items-center group cursor-default">
                        <kbd className="bg-stone-800 text-white font-pixel font-bold text-lg w-8 h-8 rounded flex items-center justify-center shadow-lg border border-stone-600 group-hover:-translate-y-1 transition-transform">{ing.key}</kbd>
          ...
          <aside className="flex-1 flex flex-col gap-6 pt-4 pr-4">
             <HygieneStation hygiene={gameState.hygiene} onClean={cleanKitchen} />
             <GlobalRanking currentScore={gameState.score} gameMode={gameState.gameMode} gameId={gameRunId} />
